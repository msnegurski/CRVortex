...
{
	return ((Rdat[i][j]-Rdat[i-1][j])*(Rdat[i][j]-Rdat[i-1][j])+(Idat[i][j]-Idat[i-1][j])*(Idat[i][j]-Idat[i-1][j]))/(dx*dx);
}
...
double energy(double **Rdat, double **Idat, int N, double dx)
{
	...
	for (i=1; i<N; i++)
		for (j=1; j<N; j++)
		{
			e+=gradx_sq(Rdat,Idat,i,j,dx)+grady_sq(Rdat,Idat,i,j,dx)-0.5*psi_sq(Rdat,Idat,i,j)*psi_sq(Rdat,Idat,i,j);
...
double average_width(double **Rdat, double **Idat, int N, double dx)
{
	...
	for (i=0;i<N;i++)
	{
		s+=((i-N/2)*(i-N/2)+(j-N/2)*(j-N/2))*dx*dx*psi_sq(Rdat,Idat,i,j)*dx;
...
int main()
{
	...
	*/
	...
	printf("%f %f %f %f\n",i*n*dt,average_width(RePsi,ImPsi,cnum,dx),energy(RePsi,ImPsi,cnum,dx),power(RePsi,ImPsi,cnum,dx));
	for (i=0;i<100;i++)
	{
		//printf("%f %f\n",i*n*dt,energy(RePsi,ImPsi,cnum,dx));
		//fprintf(ham,"%f %f\n",i*n*dt,energy(RePsi,ImPsi,cnum,dx));
