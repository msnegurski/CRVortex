...
int main(void)
{
    ...
    for (i = 1; i < n-1; i++)
    {
        dH[i] = ((psi_1[i+1] - psi_1[i-1]) / (2 * delta)) *
                ((psi_1[i+1] - psi_1[i-1]) / (2 * delta)) +
                ((psi_2[i+1] - psi_2[i-1]) / (2 * delta)) *
                ((psi_2[i+1] - psi_2[i-1]) / (2 * delta)) -
                theta[i] * (psi_1[i] * psi_1[i] + psi_2[i] * psi_2[i]);
    }
    ...
    FILE * fp_devsum = fopen("sequencedevsummary.txt", "w");
